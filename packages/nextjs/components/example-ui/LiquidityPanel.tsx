...
export const LiquidityPanel = () => {
  ...
  // Get deployed SimpleDEX address
  const { data: dexContract } = useDeployedContractInfo("SimpleDEX");
  const simpleDexAddress = dexContract?.address as `0x${string}` | undefined;
  ...
  const { writeAsync: approveTokenA } = useScaffoldContractWrite({
    ...
    args: [simpleDexAddress!, parseUnits("1000000", 18)],
  });

  const { writeAsync: approveTokenB } = useScaffoldContractWrite({
    contractName: "SimpleUSDC",
    functionName: "approve",
    args: [simpleDexAddress!, parseUnits("1000000", 6)],
  });
  ...
  // Expected outputs for removing liquidity
  const expectedA =
            ...
            {removeAmount && (
              <div className="alert alert-info mt-2">
                <div className="text-xs">
                  ...
                  <p>• {formatBalance(expectedA, 18)} {symbolA}</p>
                  <p>• {formatBalance(expectedB, 6)} {symbolB}</p>
                </div>
              </div>
            )}
